...
>
struct hash_impl<T, Prime, Hash, TEntropy, Index, 0> {
  static_assert(Index > 0, "expected a non-empty source");

  using result = std::integral_constant<T, Hash>;
};

template <typename T, template <std::size_t> class TEntropy, T Prime, T Seed>
using hash = typename hash_impl<T, Prime, Seed, TEntropy>::result;
...
template <typename T, T Prime, T Seed>
using mix = std::integral_constant<
  T,
  Seed ^ entropy<T>::value ^ __LINE__
    ^ hash<T, date_entropy, Prime, static_cast<T>(Seed ^ __LINE__)>::value
    ^ hash<T, time_entropy, Prime, static_cast<T>(Seed ^ __LINE__)>::value
    ^ hash<T, file_entropy, Prime, static_cast<T>(Seed ^ __LINE__)>::value
>;
 ...
 *
 *  - Prime: a prime number used during the computation of the
 *    pseudo-random numbers. Doesn't necessarily have to be a
 *    prime, but primes should yield better results.
 *
 * Example:
 *
...
template <
  ...
  T PreSeed = 0,
  T Prime = detail::compile_time_rng_impl::default_compile_time_rng_prime<
    T
  >::value
>
struct compile_time_rng {
  ...
  struct seed {
    ...
    using get = detail::compile_time_rng_impl::mix<
      T, Prime, static_cast<T>((PreSeed ^ Seed ^ __LINE__) * (Iteration + 1))
    >;
  };
