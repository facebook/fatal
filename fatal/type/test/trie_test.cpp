...
struct check_trie_find_visitor {
  template <typename String, typename NeedleChar>
  void operator ()(
    tag<String>,
    std::basic_string<NeedleChar> const &needle,
    std::size_t &matches
  ) {
    FATAL_EXPECT_EQ((to_instance<std::string, String>()), needle);
    FATAL_EXPECT_EQ(0, matches++);
  }
};

template <>
struct check_trie_find_visitor<false> {
...
template <bool ExpectMatch, typename Tree>
void check_trie_find(std::string const &needle) {
  check_trie_find_visitor<ExpectMatch> visitor;

  std::size_t matches = 0;
  auto result = trie_find<Tree>(
    needle.begin(), needle.end(), visitor, needle, matches
  );

  auto const expectedResult = ExpectMatch;
  FATAL_EXPECT_EQ(expectedResult, result);
  auto const expectedMatches = ExpectMatch ? 1 : 0;
...
FATAL_TEST(trie, find h_empty) {
  check_trie_find<false, hs_tree>("");
}

FATAL_TEST(trie, find h_h) {
  check_trie_find<true, hs_tree>("h");
}

FATAL_TEST(trie, find h_H) {
  check_trie_find<false, hs_tree>("H");
}

FATAL_TEST(trie, find h_ha) {
  check_trie_find<true, hs_tree>("ha");
}

FATAL_TEST(trie, find h_Ha) {
  check_trie_find<false, hs_tree>("Ha");
}
...
FATAL_TEST(trie, find h_hat) {
  check_trie_find<true, hs_tree>("hat");
}

FATAL_TEST(trie, find h_Hat) {
  check_trie_find<false, hs_tree>("Hat");
}
...
FATAL_TEST(trie, find h_hi) {
  check_trie_find<true, hs_tree>("hi");
}

FATAL_TEST(trie, find h_Hi) {
  check_trie_find<false, hs_tree>("Hi");
}
...
FATAL_TEST(trie, find h_hint) {
  check_trie_find<true, hs_tree>("hint");
}

FATAL_TEST(trie, find h_Hint) {
  check_trie_find<false, hs_tree>("Hint");
}
...
FATAL_TEST(trie, find h_hit) {
  check_trie_find<true, hs_tree>("hit");
}

FATAL_TEST(trie, find h_Hit) {
  check_trie_find<false, hs_tree>("Hit");
}
...
FATAL_TEST(trie, find h_ho) {
  check_trie_find<true, hs_tree>("ho");
}

FATAL_TEST(trie, find h_Ho) {
  check_trie_find<false, hs_tree>("Ho");
}
...
FATAL_TEST(trie, find h_hot) {
  check_trie_find<true, hs_tree>("hot");
}

FATAL_TEST(trie, find h_Hot) {
  check_trie_find<false, hs_tree>("Hot");
}
...
FATAL_TEST(trie, find h_hut) {
  check_trie_find<false, hs_tree>("hut");
}
